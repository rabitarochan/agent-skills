...
2. Parse the config line (first line inside the markers):
   `<!-- skdd:config prefix=<prefix> hooks=<true|false> version=<semver> -->`
   — space-separated `key=value` pairs, keys: `prefix`, `hooks`, `version`.
3. If the config line is missing or unparseable, ask the user for the prefix
   and whether the hook is installed, then continue (self-healing re-render).
...
Same procedure as setup Step 4: copy `SKILL.md`,
`references/harvest-protocol.md`, `references/adr-entry-schema.md`
(substituting `{{SKDD_PREFIX}}`, `{{SKDD_HOOKS}}`, `{{SKDD_VERSION}}` with the
Step 1 parameters and the NEW plugin version) and `.gitignore` (verbatim) from
...
- If `hooks=true`: re-copy `${CLAUDE_PLUGIN_ROOT}/templates/skdd-stop.sh` to
  `.claude/hooks/skdd-stop.sh` (overwrite). Then verify `.claude/settings.json`
...
1. Render `${CLAUDE_PLUGIN_ROOT}/templates/agents-section.md` with the Step 1
   `prefix` and `hooks` values and the NEW plugin version.
